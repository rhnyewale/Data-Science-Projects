...
## select columns
df2 <- df[,c("X","Private","Apps")]

df2

## order data
df3 <- df[order(df$Apps),]
...
## [your code here]
df3<-filter(df, Grad.Rate >= 80 & Grad.Rate <= 100)

df4<-select(df3, X,Grad.Rate)

arrange(df4,desc(Grad.Rate))
...
## Group by according to Private Values 'Yes' & 'No'
## Summarise() is really powerful when working in groups and reduces the output
...
dfc<- group_by(college,Private)
dfc<- summarise(dfc, max= max(Outstate), min= min(Outstate))
dfc
